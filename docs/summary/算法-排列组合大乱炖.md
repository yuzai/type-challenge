...
```ts
type Combo<T extends string, Prev extends string = ''> =
  T extends any
    ? Prev extends ''
      ? T | Combo<Exclude<T, T>, T> extends never ? T : never
...
```ts
type Permutation<T extends string, Copy extends string = T> =
  [T] extends [never]
    ? ''
    : T extends any
      ? `${T}${'' extends Permutation<Exclude<Copy, T>> ? '' : ` ${Permutation<Exclude<Copy, T>>}`}`
      : never;
```
...
// 稍简洁的版本：
type PowerSet<T extends any[], R extends any[] = []> =
