...
type Contains<S, U> = U extends any
  ? S extends `${infer _A}${U &
      (string | number | bigint | boolean)}${infer _B}`
    ? false
    : never
  : never;
```
...
type IsFixedCheck<S> = S extends `${infer A}${string}${infer B}` // 包含 ${string}
  ? false
